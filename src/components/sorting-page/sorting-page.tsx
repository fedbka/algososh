...
import { TElement, TSteps, getRandomNumbers, sortNumbers } from "./sorting-page-algoritm";
    ...
    <SolutionLayout title="Сортировка массива">
      <form className={styles.form} onSubmit={(e) => e.preventDefault()}>
        ...
        <fieldset className={styles.form_actions} disabled={isLoading}>
          <Button text="По возрастанию" type="button" onClick={() => startVisualization("ascending")} disabled={!values.sortingType} isLoader={isLoading && action == 'ascending'} />
          <Button text="По убыванию" type="button" onClick={() => startVisualization("descending")} disabled={!values.sortingType} isLoader={isLoading && action == 'descending'} />
        </fieldset>
        <fieldset className={styles.form_otherActions} disabled={isLoading}>
          <Button type="button" text="Новый Массив" onClick={() => newNumbers()} disabled={isLoading} />
        </fieldset>
      </form>
